...
(function($){
    ...
    $.fn.tree.defaults={url:null,method:"post",animate:false,checkbox:false,cascadeCheck:true,onlyLeafCheck:false,lines:false,dnd:false,data:null,queryParams:{},formatter:function(node){
        ...
        $.ajax({type:opts.method,url:opts.url,data:_1c9,dataType:"json",beforeSend : function(xhr) {
...
(function($){
    ...
    $(window).resize(function(){
        $("body>div.window-mask").css({width:$(window)._outerWidth(),height:document.body.scrollHeight});
    });
...
(function($){
    ...
    $.fn.datagrid.defaults=$.extend({},$.fn.panel.defaults,{sharedStyleSheet:false,frozenColumns:undefined,columns:undefined,fitColumns:false,resizeHandle:"right",autoRowHeight:true,toolbar:null,striped:false,method:"post",nowrap:true,idField:null,url:null,data:null,loadMsg:"Processing, please wait ...",rownumbers:false,singleSelect:false,ctrlSelect:false,selectOnCheck:true,checkOnSelect:true,pagination:false,pagePosition:"bottom",pageNumber:1,pageSize:10,pageList:[10,20,30,40,50],queryParams:{},sortName:null,sortOrder:"asc",multiSort:false,remoteSort:true,showHeader:true,showFooter:false,scrollbarSize:18,rowEvents:{mouseover:_621(true),mouseout:_621(false),click:_629,dblclick:_633,contextmenu:_637},rowStyler:function(_7c3,_7c4){
        ...
        $.ajax({type:opts.method,url:opts.url,data:_7c5,dataType:"json",beforeSend : function(xhr) {
            xhr.overrideMimeType("text/plain; charset=utf-8");
            xhr.setRequestHeader("Content-Type","text/plain");
        },success:function(data){
            _7c6(data);
        ...
        function changeWidth(){
            $('.datagrid').each(function() {
                var _th = this;
                var _width = $(_th).width();
                var _check = $(_th).find('.datagrid-header-check');
                var _num = $(_th).find('.datagrid-header-rownumber');
                if(_check) {
                    _check.css("width", (_width / 100) * 5 + 'px');
                    $(_th).find(".datagrid-cell-check").css("width", (_width / 100) * 5 + 'px');
                }
                if(_num) {
                    _num.css("width", (_width / 100) * 5 + 'px');
                    $(_th).find('.datagrid-cell-rownumber').css("width", (_width / 100) * 5 + 'px');
                }
            });
        }
    },onLoadError:function(){
...
(function($){
    ...
    $.extend($.fn.combobox.methods, {
        completeCheck: function (jq) {
            var textbox = $(jq.selector).combobox('textbox');
            textbox.blur(function () {
                var valueField = $(jq.selector).combobox("options").valueField;
                var val = $(jq.selector).combobox("getValue");
                var allData = $(jq.selector).combobox("getData");
                var result = true;
                ...
                if (result) {
                    $(jq.selector).combobox("clear");
                }
            })
        }
    });
    $.extend($.fn.combobox.methods, {
        overflowTips: function(jq){
            var panel=$(jq.selector).combobox("panel");
            var valuePanel=panel.find("div.combobox-item");
            valuePanel.each(function(){
                $(this).on("mouseover",function(){
                    var value=$(this).text();
                    if(this.offsetWidth < this.scrollWidth){
                        $(this).tooltip({
                            position: 'top',
                            content:'<span style="color:#333;">'+value+'</span>',
                            deltaY:20,
                            deltaX:14,
                            onShow: function() {
                                var t = $(this);
...
(function($) {
    ...
    aiSelect.prototype.selected = function(values) {
        //获取顶级元素
        var _el = this.$element.find(".ui-select-item");
        //获取是否是单选
        var _flag = $(this.$element).hasClass("radio");
        if(values instanceof Array) {
            ...
            for(var i = 0; i < values.length; i++) {
                if(this.block == "block") {
                    ...
                    for(var i = 0; i < _el.length; i++) {
                        var _val = $($(_el[i]).find("span")[1]).attr("value");
                        if(_val == values) {
                            $(_el[i]).addClass("checked");
                        }
                    }
                }
            ...
            if(this.block == "block") {
                ...
                if(_flag) {
                    this.$element.find(".ui-select-item[value='" + values + "']").siblings().removeClass("checked");
                }
            } else {
                for(var i = 0; i < _el.length; i++) {
                    var _val = $($(_el[i]).find("span")[1]).attr("value");
                    if(_val == values) {
                        $(_el[i]).addClass("checked");
                        if(_flag) {
                            $(_el[i]).siblings().removeClass("checked");
                            return;
                        }
                    }
                }
...
AIKeyTip.prototype = {
    init:function(options){
        ...
        var _tip = $("<div></div>").addClass("ui-keytip").css("width",_.opts.width).css("min-width","770px").css("margin-right",_.opts.width/2);
        if(_.opts.idName == ''){
...
;(function($){
    $.extend($.fn.datagrid.methods,{
        datagridCellTips:function(jq,option,param){
            if(option.field=="all"){
                var panel=$(jq.selector).datagrid("getPanel");
                var datagridCell=panel.find("div.datagrid-cell");
            }else if(param instanceof Array){
                $.each(param, function(index, item) {
                    var td=$('td[field='+field[index]+']');
                    datagridCell=td.find("div.datagrid-cell");
                })
            }else{
                var td=$('td[field='+option.field+']');
                datagridCell=td.find("div.datagrid-cell");
            }
            return datagridCell.each(function(){
                var value=$(this).text();
